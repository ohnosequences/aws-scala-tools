...
import ohnosequences.saws.signing.v3.{v3, V3SigningProcess}

import java.util.concurrent.CountDownLatch
import ohnosequences.saws.signing.{Signer, Credentials}
import ohnosequences.saws.signing.dispatchSign.{Dispatch3Implicits, DispatchUtils}
...
//      if (listObjectsRequest.getMaxKeys != null && listObjectsRequest.getMaxKeys.intValue >= 0) request.addParameter("max-keys", listObjectsRequest.getMaxKeys.toString)
  //  s3client.invoke(request, new Unmarshallers.ListObjectsUnmarshaller, listObjectsRequest.getBucketName, null)
    ...
    import Dispatch3Implicits._
    val signer = new Signer[v3.type, V3SigningProcess.type](V3SigningProcess)


    val signedHeader = signer.sign(dispatchRequest.build(), credentials)
    println(signedHeader)
